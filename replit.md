...
**Admin Features (October 31, 2025):**
- **User Photo Management Dashboard**: Admin-only page (/admin/users) displaying all users with uploaded photos. Features include LEFT JOIN query aggregating profile_media into user records, search by name/email, tier-based filtering (Free/Casual/Active/Dating/Business), and stats dashboard showing total users, users with photos, and paid members. Photo grid displays all uploaded media with sort order labels and type indicators (photo/video). Membership tier normalization handles null values, color-coded tier badges, and moderation status highlights. API endpoint (/api/admin/users-with-photos) enforces admin role requirement with 403 for non-admin access. Database stores photos in profile_media table (id, user_id, type, url, sort_order) with primary_photo_url cached in auth_users for performance.
