...
export default function Profile() {
  ...
  // Load existing profile data and media
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
  ...
  const onSave = useCallback(async () => {
    ...
    try {
      ...
      // Count new items that need upload
      const newPhotos = photos.filter(p => !p.existing);
      const newVideo = (videoAsset && videoAccepted && !videoAsset.existing) ? 1 : 0;
      const total = newPhotos.length + newVideo;
      setProgress({ done: 0, total });

      // Process photos: keep existing ones, upload new ones
      for (let i = 0; i < photos.length; i++) {
        const photo = photos[i];
        if (photo.existing) {
          // Keep existing photo
          media.push({ type: "photo", url: photo.uri, sort_order: i });
        } else {
          // Upload new photo
      ...
      if (videoAsset && videoAccepted) {
        if (videoAsset.existing) {
          // Keep existing video
          media.push({ type: "video", url: videoAsset.uri, sort_order: media.length });
        } else {
          // Upload new video
      ...
      if (refreshRes.ok) {
        ...
        const refreshMedia = refreshData?.media || [];
        
        // Update all fields with fresh data
        if (refreshUser?.name) setName(refreshUser.name);
