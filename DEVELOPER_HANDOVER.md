...
## Feature Documentation (January 27, 2026)

### One-Click Extension Billing

**Purpose:** Enables instant video call extension purchases without re-entering card details.

**Database Schema:**
```sql
ALTER TABLE auth_users ADD COLUMN default_payment_method_id VARCHAR(255);
```

**Flow:**
1. User accepts extension request
2. API checks for `default_payment_method_id` in `auth_users`
3. If saved card exists: Creates off-session PaymentIntent with `confirm: true`
4. If one-click succeeds: Extension completes immediately, timer updates
5. If one-click fails: Falls back to PaymentIntent + Stripe Elements flow with `oneClickFailed` flag
6. First successful manual payment saves card via `setup_future_usage: "off_session"`

**Key Files:**
- `/api/video/sessions/[id]/extensions/[extId]/route.js` - Accept logic with one-click attempt
- `/api/video/sessions/[id]/extensions/[extId]/confirm/route.js` - Payment confirmation + card saving
- `apps/web/src/app/(app)/video/call/page.jsx` - Frontend handles `oneClickSuccess` response

**Pricing:**
- Extension cost: $8.00 (800 cents)
- Extension duration: 10 minutes
- Grace period: 20 seconds

### Safety Reporting System

**Purpose:** Allows users to report inappropriate behavior during video calls, with admin review workflow.

**Database Schema:**
```sql
CREATE TABLE safety_reports (
  id SERIAL PRIMARY KEY,
  reporter_id INTEGER REFERENCES auth_users(id),
  reported_user_id INTEGER REFERENCES auth_users(id),
  video_session_id INTEGER REFERENCES video_sessions(id),
  reason TEXT NOT NULL,
  context VARCHAR(50) DEFAULT 'video_call',
...
**Escalation Logic:**
- Each report increments `block_count` on reported user
- When `block_count >= 3`: User flagged for admin review (`flagged_for_admin = true`)

**Admin Actions:**
- **Resolve:** Marks report as reviewed, no action taken
- **Ban User:** Sets reported user's `account_status = 'banned'`

**Key Files:**
- `/api/safety-reports/route.js` - POST (create report), GET (admin list)
- `/api/safety-reports/[id]/route.js` - PATCH (resolve/ban actions)
- `apps/web/src/app/admin/page.jsx` - Admin dashboard with Safety Reports section

### Discovery Navigation Strategy

**Purpose:** All video call exits redirect to Discovery page for continuous user engagement.

**Updated Navigation Paths:**
| Exit Point | Previous Destination | New Destination |
|------------|---------------------|-----------------|
| End Call | `/messages/{matchId}` | `/discovery` |
| Report & End | `/messages/{matchId}` | `/discovery` |
| Block & End | `/matches` | `/discovery` |
| Save Note | `/messages/{matchId}` | `/discovery` |
| Skip Note | `/messages/{matchId}` | `/discovery` |
| Grace Timeout | `/messages/{matchId}` | `/discovery` |
| Error Page | `/messages/{matchId}` | `/discovery` |

**Rationale:** Keeps users actively searching for new matches rather than returning to an existing conversation thread.

---

## High-Fidelity Production Components

The following components are the primary production assets and should be used consistently:

| Component | File | Purpose |
|-----------|------|---------|
| **DiscoveryCard** | `apps/web/src/app/(app)/discovery/page.jsx` | Swipeable profile cards with gradient overlays |
| **MatchBadge** | `apps/web/src/components/MatchBadge.jsx` | Gold star indicator for mutual matches |
| **AppHeader** | `apps/web/src/components/AppHeader.jsx` | Consistent navigation header with purple branding |
| **RewardStatusBanner** | `apps/web/src/components/RewardStatusBanner.jsx` | Rolling monthly rewards display |

---

*Document updated by Replit Agent - January 27, 2026*
